...
export function createApp() {
  ...
  const aiLimiter = rateLimit({
    windowMs: envInt("MIKE_AI_RATE_LIMIT_WINDOW_MS", 15 * 60 * 1000),
    max: envInt("MIKE_AI_RATE_LIMIT_MAX", 60),
    standardHeaders: true,
    legacyHeaders: false,
  ...
  app.use("/chat", (req, res, next) => {
    if (
      req.method === "POST" &&
      (req.path === "/" || /\/[^/]+\/generate-title$/.test(req.path))
    ) {
  ...
  app.use("/projects/:projectId/chat", aiLimiter, projectChatRouter);
  app.use("/matters/:projectId/chat", aiLimiter, projectChatRouter);
  app.use("/single-documents", documentsRouter);
  app.use("/tabular-review", (req, res, next) => {
    if (
      req.method === "POST" &&
      /\/[^/]+\/(generate|regenerate-cell|chat)$/.test(req.path)
    ) {
      return aiLimiter(req, res, next);
    }
    return next();
